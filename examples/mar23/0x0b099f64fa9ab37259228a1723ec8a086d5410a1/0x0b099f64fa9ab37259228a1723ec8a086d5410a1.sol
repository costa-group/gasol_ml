...
contract YolkChef is Ownable, ReentrancyGuard {
    ...
    // YOLK LP address
    IUniswapPair public yolkLp;
    // YOLK tokens reward per block.
    ...
    // The block number when YOLK mining starts.
    ...
    // Update the given pool's YOLK allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Yolk on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(yolk)) {
            lpSupply = yolk.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 yolkReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (yolkReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 yolkReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (yolkReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to YolkChef for YOLK allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (address(pool.lpToken) == address(yolk)) {
            _amount = yolk.fragmentToYolk(_amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(yolk)) {
            _amount = yolk.yolkToFragment(_amount);
        }
