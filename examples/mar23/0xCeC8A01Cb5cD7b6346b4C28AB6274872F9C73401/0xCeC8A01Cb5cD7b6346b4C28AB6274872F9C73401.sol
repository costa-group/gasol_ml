...
// File: contracts/ETHVaultImplementationV1.sol
...
contract ETHVaultImplementationV1 is Initializable, UUPSUpgradeable, OwnableUpgradeable, VaultBase {
    ...
    function depositToVault(uint256 vaultId, uint256 amountAmerG) external payable {
        ...
        unchecked { 
            totalEth = vault.amountCollateral + msg.value;
        ...
        vault.amountCollateral = totalEth;
    ...
    function refinance(uint256 vaultId, uint256 collateralPercentage) external payable {
        ...
        uint256 total = (ethExchangeRate() * vault.amountCollateral * 100) / collateralPercentage;
        uint256 fee;
        uint256 claimable;
    ...
    function collectInterest(uint256[] calldata vaultIds) external payable onlyOwner {
        ...
        for(uint256 i; i < vaultIds.length; ) {
            ...
            if(block.timestamp - vaults[id].lastPaymentTimestamp > 90 days) {
                uint256 amountCollateral = totalInterest(id) / ethExchangeRate();
