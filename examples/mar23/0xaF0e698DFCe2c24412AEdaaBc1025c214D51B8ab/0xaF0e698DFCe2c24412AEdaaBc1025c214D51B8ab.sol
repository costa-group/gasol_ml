...
contract CartoonisticPuzzlePieces is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    ...
    uint256 public total_ps_limit = 720;

    uint256 public max_per_wallet = 1;

    bool public wl_mint_status = true;  
    bool public refundActivity = false;  
    uint256 public whitelistCost = 0 ether;
    uint256 public total_wl_count;
    bytes32 public whitelistSigner;
    uint256 public total_wl_limit = 80;

    uint256 public accessRate = 27;

    uint256 public lastWithdrawnTime;
    uint256 public timeLimit = 86400;
    uint256 public projectStartDate;
    uint256 public fundReleasingPeriod = 2592000;

    mapping(address => uint256) public mintedRefundID;  
    mapping(uint256 => address) public refundTokenIDWithOwner;  
    ...
    function mint() public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            require(balanceOf(msg.sender) + 1 <= max_per_wallet,"Per wallet minting limit reached");
        ...
        refundTokenIDWithOwner[totalSupply()] = msg.sender;   
        mintedRefundID[msg.sender] = totalSupply();    
    ...
    function whitelistMint(bytes32[] calldata  _proof) payable public{
        ...
        require(balanceOf(msg.sender) + 1 <= max_per_wallet,"Per wallet minting limit reached");
        require(total_wl_count + 1 <= total_wl_limit, "Whitelist Limit Reached");

        require(msg.value >= whitelistCost * 1, "insufficient funds");
        ...
        total_wl_count = total_wl_count + 1; 
    ...
    function toggleRefundActivity() external onlyOwner {
        
        if(refundActivity==false){
            refundActivity = true;
        }else{
    ...
    function dailyWithdraw() external payable onlyOwner {

    require((block.timestamp - lastWithdrawnTime) >= timeLimit, "Waiting time not yet passed");
    
    (bool main, ) = payable(owner()).call{value: address(this).balance * ((accessRate /10000)) * ((block.timestamp - lastWithdrawnTime)/86400) }("");
    require(main);
    ...
    function setAccessRate(uint256 _newAccessRate) external onlyOwner {
        accessRate = _newAccessRate;
    }  

    function setWhitelistSigner(bytes32 _newWhitelistSigner) external onlyOwner {
    ...
    function setTotal_ps_limit(uint256 _total_ps_limit) external onlyOwner {
    ...
    function setFundReleasingPeriod(uint256 _fundReleasingPeriod) external onlyOwner {
        fundReleasingPeriod = _fundReleasingPeriod;
    }
    ...
    require(refundActivity == true, "Refund Activity is Over");    
    require((block.timestamp - projectStartDate) >= fundReleasingPeriod, "Waiting time not yet passed");
    require(publicSaleCost <= 1 ether, "Minting price is over the Margin, So you cannot get the refund");
    require(ownerOf(mintedRefundID[msg.sender]) == msg.sender, "You do not have the ownership of your refund token");
       uint256 refundAmount =  1 ether;
        (bool user, ) = payable(msg.sender).call{value: refundAmount}("");
        require(user);
    }
}