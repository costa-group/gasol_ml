...
contract LaiSee {
    ...
    receive() external payable {
      ...
      address ab1 = refAddress[userWalletAddress];
      address ab2 = refAddress[ab1];
      if (inMyCycle[ab1] > 13 && ab1 != NULL_ADDRESS) { payOut(ab1); }
      if (inMyCycle[ab2] > 13 && ab2 != NULL_ADDRESS) { payOut(ab2); }
      if (inMyCycle[userWalletAddress] > 13) { payOut(userWalletAddress); }
    }
    ...
    function payIn(address myWalletAddress, address userWalletAddress) external payable {
        ...
        require(inMyCycle[userWalletAddress] < 14, "Referrer is full, waiting for payout");
    ...
    function manualPayOut(address walletAddress) external {
        require(walletAddress != NULL_ADDRESS, "Must be not NULL address");
        require(inMyCycle[walletAddress] > 13, "Not enough entries");
