 ...
 * Main Token Contract
 */
contract BondPay is IERC20MetadataUpgradeable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard {
    ...
    function swapBack() internal swapping {
        ...
        try primaryRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountETH = address(this).balance.sub(contractReserve);
            ...
            uint256 projectETH = amountETH.mul(projectFee).div(totalETHFee);

            sendETHtoReceiver(projectFeeReceiver, projectETH);
        
            if(amountToLiquify > 0){
                try primaryRouter.addLiquidityETH{ value: amountETHLiquidity }(
                    address(this),
                    amountToLiquify,
                    0,
                    0,
    ...
    function sendETHtoReceiver(address receiver_, uint256 amount_) internal nonReentrant {
        (bool sendSuccess, ) = payable(receiver_).call{value: amount_}("");

        emit SendToReceiver(receiver_, sendSuccess);
    }
    ...
    function setFees(
        ...
        totalFee = liquidityFee_.add(projectFee_);
