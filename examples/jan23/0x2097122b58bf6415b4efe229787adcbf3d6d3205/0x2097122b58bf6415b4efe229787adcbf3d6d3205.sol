...
contract UVTakeProfit is Ownable, AccessControl, IUVTakeProfit {
    ...
    function distributeTokens(
        ...
        for (uint256 index = 0; index < _users.length; index++) {
            ...
            TransferHelper.safeTransfer(tokenAddress, user, amountToUser.sub(fee).div(dec));
        }
        if (totalFee > 0) TransferHelper.safeTransfer(tokenAddress, fundWallet, totalFee.div(dec));
    }
...
contract UVReserve is Ownable, AccessControl, IUVReserve {
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        require(amount > 0);
        TransferHelper.safeTransfer(token, tpPool, amount);
    }
