...
contract LootGPTDiceRoll is VRFV2WrapperConsumerBase {
    event DiceRollRequest(uint256 requestId);
    event DiceRollResult(uint256 requestId, uint256 outcome);
    ...
    function roll(uint256 sides) external returns (uint256) {
        ...
        override
    {
        ...
        statuses[requestId].outcome = (randomWords[0] % statuses[requestId].sides) + 1;

        emit DiceRollResult(requestId, statuses[requestId].outcome);
    }
