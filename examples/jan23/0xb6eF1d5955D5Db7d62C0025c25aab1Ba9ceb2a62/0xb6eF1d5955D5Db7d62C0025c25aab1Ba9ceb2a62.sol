...
contract Quadron is IQuadron, ERC20, ReserveFormula {
    ...
    // Declare constants
    uint32 internal constant WEIGHT = 333e3; // reserve ratio = weight / 1e6.  200e3 corresponds to 20%
    ...
    function sell(uint256 amount) external override {
        ...
        if(address(this).balance > 0) {
            ...
            if(approvedToken[msg.sender]) {
                ...
                if(earlySellPenalty) {
                    // early sale penalty is 99% of non-ratio price
                    proceeds = amount * address(this).balance / (100 * startSupply);
                } else {
    ...
    function quoteAvgPriceForTokens(uint256 amount) external view override returns (uint256 avgPriceForTokens) {
        ...
        if(address(this).balance == 0 || totalSupply() == 0) {
            ...
            if(approvedToken[msg.sender]) {
                ...
                if(earlySellPenalty) {
                    // selling early results in reduced price
                    return avgPrice * 1 / 100;
                } else {
                    return avgPrice;
                }
            }
