...
contract DFreelance is Ownable, ReentrancyGuard { 
    ...
    function getProjectsSize() external view onlyObservers returns (uint){
        return projectidList.length;
    }

    function getProjectId(uint idx) external view onlyObservers returns(uint) {
    ...
    function getMilestoneId(uint _projectId, uint _idx) public view onlyObservers returns(uint) {
        return milestoneIdList[_projectId][_idx];
    }

    function getMilestonesSize(uint _projectId) public view onlyObservers returns(uint) {
    ...
    function settleDisputeMilestone(uint _projectId, uint _milestoneId, uint _disputeId) private
    {
        ...
        if(tokenAddr == address(0)) {
          ...
          if(paymentInfo.freelancerAmount > 0) {
            ...
            IERC20(milestones[_projectId][_milestoneId].token).safeTransfer(treasury, paymentInfo.freelancerFeeAmt);
          }
    ...
    {
        require(msg.sender == projects[_projectId].client || msg.sender == projects[_projectId].freelancer, "Invalid access");     
        require( disputes[_projectId][_disputeId].defendantUpdatedAt == 0 && (block.timestamp - disputes[_projectId][_disputeId].createdAt) > delayTimes[DelayType.Dispute], "You can accept the dispute payment after 48 hours.");
