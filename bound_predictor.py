...
def export_to_onnx(model_filename):
    sfs_builder = SFSGraph(node_features='multi_push',regression=True)
    model = torch.load(Path(__file__).parent.joinpath(Path(model_filename)).resolve())
    model.eval()
    ...
    x, edge_index = sfs_builder.build_graph_from_sfs(block_sfs)
