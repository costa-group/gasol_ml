...
class Model_3(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        if args.rnn_class == 'lstm':
            self.rnn = LSTM(vocab_size if self.emb is None else embed_dim, hidden_channels, num_layers=layers, dropout=args.rnn_drop_out_p, bidirectional=True)
            self.rnn_type = 'lstm'
        elif args.rnn_class == 'gru':
            self.rnn = GRU(vocab_size if self.emb is None else embed_dim, hidden_channels, num_layers=layers, dropout=args.rnn_drop_out_p, bidirectional=True)
            self.rnn_type = 'gru'
        else:
            raise Exception(f'Unknown RNN: {args.rnn_class}')

        #hidden_channels += self.vocab_size-1
        self.lin = Linear(2*hidden_channels, out_channels)
        self.lin1 = Linear(2*hidden_channels, 2*hidden_channels)
        self.lin2 = Linear(2*hidden_channels, 2*hidden_channels)
    ...
    def __build_features_vec_2(self,seq):
        features = [0]*self.vocab_size
        for token in seq:
            features[ token ] = 1
        return features[1:]
    ...
    def forward(self, data):
        ...
        #bag = torch.tensor([ self.__build_features_vec_2(seq) for seq in x.tolist() ]).to(torch.float)
